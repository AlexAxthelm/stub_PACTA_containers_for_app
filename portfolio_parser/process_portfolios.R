...
log_debug("Found %i files", length(files_to_process))

for (f in files_to_process) {

  log_info("Processing file: %s", f)
  contents <- read.csv(
    file = f,
    header = TRUE
  )
  if (nrow(contents) == 0) {
    log_warn("File %s is empty", f)
    next
  }
  ...
  log_debug("writing to file: %s", output_file)
  writeLines(
    text = contents_json,
    con = output_file
  )
  ...
  log_info("Source file %s processed to %s", f, output_file)
